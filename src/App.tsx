          ...
          {user ? (
            <>
              <div className="text-right">
                ...
                <p className="text-[10px] text-green-600 font-medium uppercase tracking-widest">
                  Sistema Online
                </p>
              </div>
              <button
        ...
        {isOpen && (
          ...
          >
            ...
            {!user && (
              <button
                onClick={signInWithGoogle}
                className="w-full bg-indigo-600 text-white p-3 rounded-lg font-bold uppercase tracking-widest text-xs"
              >
                Entrar
              </button>
            )}
            {user && (
              <button
                onClick={logout}
                className="w-full text-red-500 font-bold uppercase tracking-widest text-xs text-left"
              >
                Sair
              </button>
            )}
...
const CourseCard = ({
  ...
  const handleConfirmDelete = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      await Promise.race([onDelete(course.id), timeout]);
    } catch (err: any) {
    ...
    >
      {isClosed && !showConfirm && (
        <div className="absolute top-0 left-0 right-0 bg-slate-900/10 backdrop-blur-[2px] h-full z-10 pointer-events-none flex items-center justify-center">
          <span className="bg-slate-900 text-white text-[10px] font-black uppercase tracking-[0.2em] px-4 py-2 rounded-full shadow-xl">
            Desativado
          </span>
        </div>
      )}
      ...
      {showConfirm && (
        <div className="absolute inset-0 z-30 bg-rose-600/95 backdrop-blur-sm flex flex-col items-center justify-center p-6 text-center">
          ...
          <p className="text-white font-black uppercase tracking-tighter text-sm mb-4">
            Excluir este curso?
          </p>
      ...
      <div className="aspect-[16/10] bg-slate-100 relative overflow-hidden flex items-center justify-center">
        ...
        <div className="absolute top-4 left-4 flex flex-col gap-2">
          ...
          {isAdmin && (
            <span
              className={`text-[9px] uppercase tracking-widest font-black px-2.5 py-1 rounded border self-start ${isClosed ? "bg-rose-50 text-rose-600 border-rose-100" : "bg-emerald-50 text-emerald-600 border-emerald-100"}`}
            >
      ...
      <div className="p-6">
        <h3 className="text-lg font-bold text-slate-900 leading-tight mb-2 group-hover:text-indigo-600 transition-colors uppercase tracking-tight">
          {course.title}
        </h3>
        <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest mb-4 flex items-center gap-2">
          <Users className="w-3.5 h-3.5 text-indigo-500" /> Professor:{" "}
          <span className="text-slate-900">
            {course.instructor || "A definir"}
          </span>
        </p>
          ...
          <div className="flex flex-col gap-1">
            <div className="flex items-center gap-2 text-slate-400">
              ...
              <span className="text-[9px] font-bold uppercase tracking-widest">
                {course.duration || "Consulte"}
              </span>
            </div>
            <div className="flex items-center gap-2 text-slate-400">
              <User className="w-3 h-3" />
              <span className="text-[9px] font-bold uppercase tracking-widest">
                {course.capacity || 0} Vagas
              </span>
            </div>
          </div>
          {onEnroll && (
...
const StatusBadge = ({ status }: { status: Enrollment["status"] }) => {
  ...
  return (
    <span
      className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-[9px] font-black border-2 tracking-[0.1em] shadow-sm ${config.color}`}
    >
...
const AppContent = () => {
  ...
  const [migratingStudentData, setMigratingStudentData] = useState<
    Enrollment["studentData"] | null
  >(null);
  const [isPreviewingPrint, setIsPreviewingPrint] = useState(false);
  const [isPreviewingFullEnrollmentPrint, setIsPreviewingFullEnrollmentPrint] =
    useState(false);
  const [isPreviewingClassPrint, setIsPreviewingClassPrint] = useState(false);
  const [isPreviewingFullDataPrint, setIsPreviewingFullDataPrint] =
    useState(false);
  ...
  const formatSaneamento = (val: string | undefined) => {
    ...
    if (upper === "NÃO" || upper === "IRREGULAR" || upper === "NAO")
      return "Irregular";
    return val;
  };
  ...
  const groupedEnrollments = filteredEnrollments.reduce(
    (acc, enrollment) => {
  ...
  const occupancyPercentage =
    totalCapacity > 0
      ? Math.min(100, Math.round((totalApprovedCount / totalCapacity) * 100))
      : 0;
  ...
  const handleStatusUpdate = async (
    ...
    try {
      ...
      fetchAllEnrollments();
      showError(
        "Erro ao atualizar status. O sistema pode estar com limite de uso atingido.",
      );
    }
  };
  ...
  const handleCourseDelete = async (id: string) => {
    ...
    try {
      ...
      await Promise.race([deleteCourse(id), timeoutPromise]);
      ...
      if (error.message && error.message.includes("Timeout")) {
        console.warn(
          "Delete pending on server due to slowness. Keeping optimistic UI state.",
        );
        return;
      }
      ...
      let errorMsg =
        "Não foi possível confirmar a exclusão. O curso foi restaurado para segurança.";
      if (
        error.message?.includes("Quota exceeded") ||
        error.message?.includes("resource-exhausted")
      ) {
        errorMsg =
          "Limite de uso do Firebase atingido. O curso não pôde ser excluído no servidor.";
      }
    ...
    showConfirm(
      ...
      async () => {
        ...
        try {
          ...
          await Promise.race([deleteEnrollment(id), timeoutPromise]);
          await fetchStats();
          ...
          if (error.message && error.message.includes("Timeout")) {
            console.warn(
              "Enrollment delete pending on server. Keeping optimistic UI state.",
            );
            return;
          }
          ...
          let errorMsg =
            "Não foi possível confirmar a exclusão. A matrícula foi restaurada.";
          if (
            error.message?.includes("Quota exceeded") ||
            error.message?.includes("resource-exhausted")
          ) {
  ...
  const handleStudentDeleteAllData = async (
    studentCpf: string | undefined,
    ...
    if (enrollmentsToDelete.length === 0) {
      showError(
        "Nenhum cadastro ou matrícula associada a este aluno foi encontrada para exclusão.",
      );
      return;
    }
    ...
    showConfirm(
      ...
      async () => {
        ...
        try {
          const studentIdKeys = Array.from(
            new Set(
              enrollmentsToDelete
                .map((e) => {
          ...
          await Promise.all([fetchStats(), fetchAllEnrollments()]);
          ...
          try {
            ...
            console.warn(
              "Falha ao criar log de auditoria para exclusão total de dados",
              logErr,
            );
          }

          showSuccess(
            "Todas as informações e matrículas do aluno foram excluídas permanentemente.",
          );
        } catch (error: any) {
          console.error("Erro ao excluir dados totais do aluno:", error);
          if (error.message && error.message.includes("Timeout")) {
            console.warn(
              "Lote de remoção de dados pendente no servidor. Mantendo interface otimista.",
            );
            return;
          }
          ...
          setAllEnrollments(previousEnrollments);
          showError(
            "Erro ao excluir todas as informações do aluno. O cadastro foi restaurado.",
          );
        }
      },
      true,
  ...
  const handleAcceptTerms = async (metadata: {
    ip: string;
    version: string;
  }) => {
    if (user) {
      ...
      try {
        ...
        console.warn(
          "Aviso: Falha ao salvar no banco (Cota?), mas salvo localmente.",
        );
      }
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800">
        ...
        <section className="lg:w-1/3 bg-slate-100 border-r border-slate-200 p-8 sm:p-12 lg:p-16 flex flex-col justify-between">
          ...
          >
            ...
            <p className="text-slate-500 text-sm leading-relaxed mb-12 max-w-sm">
              Ferramenta interna para cadastro de novos alunos. Realize a
              inscrição coletando os dados do jovem e do responsável no balcão.
            </p>
            ...
            <div className="grid grid-cols-1 gap-4 mb-12">
              <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm transition-transform hover:scale-[1.02]">
                <p className="label-caps mb-1">
                  Alunos Atendidos{" "}
                  {selectedPolo === "Todos" ? "" : `(${selectedPolo})`}
                </p>
                <div className="flex justify-between items-end">
                  <span className="text-3xl font-black text-slate-900">
                    {studentList.length}
                  </span>
                  <span className="text-[10px] text-indigo-600 font-bold bg-indigo-50 px-2 py-1 rounded tracking-widest uppercase">
                    ÚNICOS
                  </span>
                </div>
              </div>
              <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm transition-transform hover:scale-[1.02]">
                ...
                <div className="flex justify-between items-end">
                  ...
                  <span className="text-[10px] text-emerald-600 font-bold bg-emerald-50 px-2 py-1 rounded tracking-widest uppercase">
                    CONTRATOS
                  </span>
                </div>
              </div>
              <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm transition-transform hover:scale-[1.02]">
                <p className="label-caps mb-1">
                  Cursos {selectedPolo === "Todos" ? "Totais" : selectedPolo}
                </p>
                <div className="flex justify-between items-end">
                  <span className="text-3xl font-black text-slate-900">
                    {filteredCourses.length}
                  </span>
                  <span className="text-[10px] text-amber-600 font-bold bg-amber-50 px-2 py-1 rounded tracking-widest uppercase">
                    OFERTA
                  </span>
                </div>
              </div>
            ...
            {user && (
              <div className="space-y-4">
                {scholarships.find((s) => s.studentId === user.uid)?.status ===
                  "active" && (
                  <motion.div
                    initial={{ opacity: 0, y: 10 }}
                    animate={{ opacity: 1, y: 0 }}
                  ...
                  >
                    <div className="relative z-10">
                      ...
                      <p className="text-[9px] font-bold uppercase tracking-widest opacity-70">
                        Benefício Mensal Liberado
                      </p>

                      {scholarshipConfig?.benefits &&
                        scholarshipConfig.benefits.length > 0 && (
                          <div className="mt-6 pt-4 border-t border-white/10 flex flex-wrap gap-2">
                            {scholarshipConfig.benefits.map((b, i) => (
                              <span
                                key={i}
                                className="bg-white/10 px-2 py-1 rounded text-[8px] font-black uppercase tracking-widest border border-white/5"
                              >
                                {b}
                              </span>
                            ))}
          ...
          <div className="bg-indigo-900 p-8 rounded-2xl text-white shadow-2xl relative overflow-hidden group">
            <div className="relative z-10">
              <p className="text-[10px] opacity-60 mb-2 uppercase tracking-widest font-bold">
                Próximo Ciclo
              </p>
              ...
              <p className="text-[10px] mt-3 opacity-50 font-bold tracking-widest uppercase">
                {courses.length > 0
                  ? `${occupancyPercentage || 0}% das turmas preenchidas`
        ...
        <section className="flex-1 p-8 sm:p-12 lg:p-16 overflow-y-auto">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-10 gap-6">
                <div>
                  ...
                  <p className="text-slate-500 font-medium text-sm">
                    Selecione uma área para iniciar sua inscrição
                  </p>
                </div>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h4 className="text-sm font-black text-indigo-900 uppercase">
                          Escolhendo novo curso para:
                        </h4>
                        <p className="text-xl font-black text-indigo-600">
                          {migratingStudentData.fullName}
                        </p>
                      </div>
                    </div>
                    <button
                ...
                {filteredCourses.length > 0 ? (
                  filteredCourses.map((course) => (
                    <CourseCard
                      ...
                      course={course}
                      onEnroll={
                        isAdmin ? () => setActiveCourse(course) : undefined
                      }
                      onEdit={
                        isAdmin ? () => setEditingCourse(course) : undefined
                      }
                      onStatusChange={
                        isAdmin ? handleCourseStatusChange : undefined
                      }
                  ...
                  <div className="col-span-full py-20 border-2 border-dashed border-slate-200 rounded-2xl flex flex-col items-center justify-center text-center">
                    ...
                    <p className="text-slate-500 text-sm max-w-xs mt-2 mb-8">
                      Comece cadastrando os cursos da sua instituição para
              ...
              <div className="pb-12 border-b border-slate-200">
                <div className="flex items-center justify-between mb-8">
                  <div>
                    ...
                    <p className="text-slate-500 font-medium text-sm">
                      Visualize a listagem de alunos por polo:{" "}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {filteredCourses.map((course) => {
                    const approvedCount = allEnrollments.filter(
                      (e) =>
                        e.courseId === course.id && e.status === "approved",
                    ).length;
                    ...
                    return (
                      ...
                      >
                        <h4 className="font-bold text-slate-900 group-hover:text-indigo-600 transition-colors mb-4">
                          {course.title}
                        </h4>
              ...
              <div id="admin" className="pt-12">
                ...
                {studentList.length > 0 ? (
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full text-left font-sans text-sm">
                        <thead className="bg-slate-50/50 border-b-2 border-slate-100">
                          <tr>
                            <th className="px-8 py-5 text-[11px] font-black uppercase tracking-[0.2em] text-slate-950">
                              Estudante
                            </th>
                            <th className="px-8 py-5 text-[11px] font-black uppercase tracking-[0.2em] text-slate-950">
                              Cursos
                            ...
                            <th className="px-8 py-5 text-[11px] font-black uppercase tracking-[0.2em] text-slate-950 text-right">
                              Ações
                            </th>
                          </tr>
                        </thead>
                        <tbody className="divide-y-2 divide-slate-50">
                          {studentList.map((studentEnrollments) => {
                            ...
                            const approvedCount = studentEnrollments.filter(
                              (e) => e.status === "approved",
                            ).length;
                            const pendingCount = studentEnrollments.filter(
                              (e) => e.status === "pending",
                            ).length;
                            const studentKey =
                              enrollment.studentId &&
                              enrollment.studentId !== "presential_referral"
                                ? enrollment.studentId
                                : enrollment.studentData.cpf
                                  ? enrollment.studentData.cpf.replace(
                                      /\D/g,
                                      "",
                                    )
                            ...
                            return (
                              <tr
                                key={
                                  enrollment.studentData.cpf || enrollment.id
                                }
                                ...
                                <td className="px-8 py-6">
                                  <div className="flex items-center gap-4">
                                    ...
                                    {scholarship?.status === "active" && (
                                      <div
                                        className="bg-emerald-50 text-emerald-600 px-3 py-1 rounded-full border border-emerald-100 flex items-center gap-2"
                                        title="Bolsista Ativo"
                                      >
                                    ...
                                    {scholarship?.status === "suspended" && (
                                      <div
                                        className="bg-rose-50 text-rose-600 px-3 py-1 rounded-full border border-rose-100 flex items-center gap-2"
                                        title="Bolsa Suspensa"
                                      >
                                ...
                                <td className="px-8 py-6">
                                  <div className="flex flex-col gap-1">
                                    ...
                                    <span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest pl-2">
                                      {approvedCount} Ativas / {pendingCount}{" "}
                                      Pendentes
                                    </span>
                                  </div>
                                </td>
                  ...
                  <div className="bg-white rounded-2xl p-12 border border-slate-200 border-dashed text-center">
                    ...
                    <h4 className="text-slate-900 font-bold uppercase tracking-tight">
                      Nenhuma Matrícula Registrada
                    </h4>
                    <p className="text-slate-500 text-sm mt-2 max-w-sm mx-auto">
                      Para matricular um aluno, localize o curso desejado na
                      lista acima e clique em{" "}
          ...
          <footer className="mt-24 pt-12 border-t border-slate-200 flex flex-col sm:flex-row justify-between items-center gap-6">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-[9px] text-slate-400 font-bold uppercase tracking-widest underline decoration-slate-200 underline-offset-4">
                Servidor: BR-Sul-01
              </span>
            </div>
          </footer>
          ...
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            <div className="relative z-10 w-full max-w-2xl">
              <EnrollmentForm
                ...
                courseTitle={
                  courses.find((c) => c.id === editingEnrollment.courseId)
                    ?.title || "Curso"
                }
              ...
              <div className="px-10 py-6 border-b border-slate-100 flex items-center justify-between bg-white relative z-10 no-print">
                <div>
                  <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight flex items-center gap-2">
                    <Eye className="w-5 h-5 text-indigo-600" /> Prévia de
                    Impressão
                  </h3>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">
                    Confira os dados antes de gerar o documento
                  </p>
                </div>
                <div className="flex items-center gap-3">
                  <button
              ...
              <div className="flex-1 overflow-y-auto p-12 bg-slate-200/50 flex justify-center custom-scrollbar">
                ...
                <div className="print-container print-area-enrollment bg-white w-full max-w-[210mm] h-[296mm] shadow-[0_20px_50px_rgba(0,0,0,0.1)] p-[2cm] relative text-slate-900 overflow-hidden">
                  ...
                  <div className="flex items-center justify-between border-b-4 border-slate-900 pb-8 mb-10">
                    <div className="flex items-center gap-4">
                      <div className="w-16 h-16 bg-slate-900 rounded-xl flex items-center justify-center text-white text-3xl font-black">
                        CB
                      </div>
                      <div>
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-xl font-mono font-bold text-slate-900">
                        #{viewingEnrollment.id.slice(0, 8).toUpperCase()}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-12">
                    ...
                    <div className="grid grid-cols-2 gap-12">
                      <section>
                        <h4 className="text-[11px] font-black text-indigo-600 uppercase tracking-[0.2em] mb-4 border-b border-indigo-100 pb-2">
                        ...
                        <div className="space-y-3">
                          <div>
                            ...
                            <p className="text-lg font-black text-slate-900 leading-tight">
                              {courses.find(
                                (c) => c.id === viewingEnrollment.courseId,
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            ...
                            <div>
                              ...
                              <p className="text-sm font-bold text-slate-900">
                                {new Date(
                                  viewingEnrollment.studentData.enrollmentDate,
                                ).toLocaleDateString()}
                              </p>
                            </div>
                          </div>
                      ...
                      <section>
                        <h4 className="text-[11px] font-black text-emerald-600 uppercase tracking-[0.2em] mb-4 border-b border-emerald-100 pb-2">
                        ...
                        <div className="space-y-3">
                          <div>
                            ...
                            <p className="text-lg font-black text-slate-900 leading-tight">
                              {viewingEnrollment.studentData.fullName}
                            </p>
                          </div>
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              ...
                              <p className="text-sm font-bold text-slate-900">
                                {viewingEnrollment.studentData.cpf ||
                                  "Não Informado"}
                              </p>
                            </div>
                            <div>
                              ...
                              <p className="text-sm font-bold text-slate-900">
                                {viewingEnrollment.studentData.birthDate}
                              </p>
                            </div>
                          </div>
                        </div>
                    ...
                    <div className="grid grid-cols-2 gap-12">
                      <section>
                        <h4 className="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4 border-b border-slate-200 pb-2">
                          Localização
                        </h4>
                        <div className="space-y-4">
                          <div>
                            ...
                            <p className="text-sm font-bold text-slate-900 leading-relaxed">
                              {viewingEnrollment.studentData.address},{" "}
                              {viewingEnrollment.studentData.houseNumber}
                            </p>
                            <p className="text-xs text-slate-500 font-medium mb-3">
                              {viewingEnrollment.studentData.neighborhood} —
                              Ref: {viewingEnrollment.studentData.landmark}
                            </p>
                            <div className="flex gap-4">
                              <div>
                                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">
                                ...
                                <p
                                  className={`text-xs font-bold ${isIrregular(viewingEnrollment.studentData.paysElectricity) ? "text-rose-600" : "text-slate-900"}`}
                                >
                                  {formatSaneamento(
                                    viewingEnrollment.studentData
                                      .paysElectricity,
                                  )}
                                </p>
                              </div>
                              <div>
                                ...
                                <p
                                  className={`text-xs font-bold ${isIrregular(viewingEnrollment.studentData.paysWater) ? "text-rose-600" : "text-slate-900"}`}
                                >
                                  {formatSaneamento(
                                    viewingEnrollment.studentData.paysWater,
                                  )}
                                </p>
                              </div>
                            </div>
                      ...
                      <section>
                        ...
                        <div className="grid grid-cols-1 gap-4">
                          <div>
                            ...
                            <p className="text-sm font-bold text-slate-900 leading-tight">
                              {viewingEnrollment.studentData.motherName}
                            </p>
                          ...
                          {viewingEnrollment.studentData.guardianName && (
                            <div>
                              <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest text-indigo-600">
                                Responsável Legal
                              </p>
                      ...
                      <div className="grid grid-cols-3 gap-8">
                        <div>
                          ...
                          <p className="text-xs font-bold text-slate-900">
                            {viewingEnrollment.studentData.schoolName}
                          </p>
                        </div>
                        <div>
                          <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">
                          ...
                          <p className="text-xs font-bold text-slate-900">
                            {viewingEnrollment.studentData.schoolGrade}
                          </p>
                        </div>
                        <div>
                          <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">
                    ...
                    <div className="pt-8 border-t-2 border-slate-100">
                      <p className="text-[10px] text-slate-400 font-medium leading-relaxed italic mb-16">
                        Declaro que as informações acima são verdadeiras e que
                        estou ciente das normas de funcionamento do projeto
                        Cursos Bantu. Este documento serve como comprovante
                        oficial de matrícula ativa quando acompanhado do carimbo
                        da instituição.
                      </p>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/40 backdrop-blur-md p-4 sm:p-8">
            ...
            >
              <div className="px-10 py-6 border-b border-slate-100 flex items-center justify-between bg-white relative z-10 no-print">
                <div>
                  <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight flex items-center gap-2">
                    <FileText className="w-5 h-5 text-indigo-600" /> Ficha
                    Cadastral Completa
                  </h3>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">
              ...
              <div className="flex-1 overflow-y-auto p-12 bg-slate-200/50 flex justify-center custom-scrollbar">
                <div className="print-container print-area-full-enrollment bg-white w-[210mm] h-[296mm] shadow-2xl p-[1.5cm] relative text-slate-900 border border-slate-100 overflow-hidden">
                  ...
                  <div className="flex items-center justify-between border-b-2 border-slate-900 pb-4 mb-6">
                    <div className="flex items-center gap-3">
                      <div className="w-12 h-12 bg-slate-900 rounded flex items-center justify-center text-white font-black text-xl">
                        CB
                      </div>
                      <div>
                        ...
                        <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">
                          Controle Administrativo - Cursos Bantu
                        </p>
                      </div>
                    </div>
                    <div className="text-right">
                  ...
                  <div className="grid grid-cols-2 gap-x-8 gap-y-6">
                    ...
                    <div className="col-span-2 bg-slate-50 p-4 rounded-lg border border-slate-200">
                      <h4 className="text-[10px] font-black uppercase tracking-widest text-indigo-600 mb-3 ml-1">
                      ...
                      <div className="grid grid-cols-3 gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-sm font-bold">
                            {viewingEnrollment.studentData.birthDate} (
                            {viewingEnrollment.studentData.age} anos)
                          </p>
                        </div>
                        <div>
                          <p className="text-[8px] font-bold text-slate-400 uppercase">
                        ...
                        <div>
                          ...
                          <p className="text-sm font-bold">
                            {viewingEnrollment.studentData.rg}
                          </p>
                        </div>
                        <div>
                          <p className="text-[8px] font-bold text-slate-400 uppercase">
                          ...
                          <p className="text-sm font-bold">
                            {viewingEnrollment.studentData.phone}
                          </p>
                        </div>
                      </div>
                    </div>
                          ...
                          <div className="pt-2 border-t border-slate-200">
                            ...
                            <div className="flex gap-4 mt-1">
                              <p className="text-[7px] font-bold uppercase">
                                CPF: {viewingEnrollment.studentData.guardianCpf}
                              </p>
                              <p className="text-[7px] font-bold uppercase">
                                RG: {viewingEnrollment.studentData.guardianRg}
                              </p>
                            </div>
                          </div>
                        )}
                    ...
                    <div className="bg-slate-50 p-4 rounded-lg border border-slate-200">
                      ...
                      <div className="space-y-3">
                        ...
                        {viewingEnrollment.studentData.integralDays && (
                          <p className="text-[7px] font-bold text-indigo-600 uppercase italic">
                            Dias Integral:{" "}
                            {viewingEnrollment.studentData.integralDays}
                          </p>
                        )}
                      ...
                      <div className="grid grid-cols-4 gap-4">
                        ...
                        <div className="col-span-4 border-t border-slate-100 pt-2">
                          ...
                          <p className="text-[10px] uppercase">
                            {viewingEnrollment.studentData.landmark || "-"}
                          </p>
                        </div>
                      </div>
                    </div>
                      ...
                      <div className="grid grid-cols-4 gap-4">
                        <div>
                          ...
                          <p className="text-[10px] font-bold uppercase">
                            {viewingEnrollment.studentData.housingCondition}
                          </p>
                        </div>
                        <div>
                          <p className="text-[8px] font-bold text-slate-400 uppercase">
                          ...
                          <p className="text-[10px] font-bold uppercase">
                            {viewingEnrollment.studentData.householdCount}
                          </p>
                        </div>
                        <div>
                          <p className="text-[8px] font-bold text-slate-400 uppercase">
                          ...
                          <p className="text-[10px] font-bold uppercase">
                            {viewingEnrollment.studentData.incomePerCapita}
                          </p>
                        </div>
                        <div>
                          <p className="text-[8px] font-bold text-slate-400 uppercase">
                          ...
                          <p className="text-[10px] font-bold uppercase">
                            {viewingEnrollment.studentData.receivesBenefit}
                          </p>
                        </div>
                        <div>
                          <p className="text-[8px] font-bold text-slate-400 uppercase">
                          ...
                          <p
                            className={`text-[10px] font-bold uppercase ${isIrregular(viewingEnrollment.studentData.paysElectricity) ? "text-rose-600" : ""}`}
                          >
                            {formatSaneamento(
                              viewingEnrollment.studentData.paysElectricity,
                            )}
                          </p>
                        </div>
                        <div>
                          ...
                          <p
                            className={`text-[10px] font-bold uppercase ${isIrregular(viewingEnrollment.studentData.paysWater) ? "text-rose-600" : ""}`}
                          >
                            {formatSaneamento(
                              viewingEnrollment.studentData.paysWater,
                            )}
                  ...
                  <div className="absolute bottom-[2cm] left-[1.5cm] right-[1.5cm]">
                    ...
                    <div className="mt-12 flex justify-between items-end grayscale opacity-30">
                      <p className="text-[7px] font-black uppercase tracking-[0.3em]">
                        Cursos Bantu - Ficha de Matrícula v2.0
                      </p>
                      <p className="text-[7px] font-mono tracking-tighter">
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/40 backdrop-blur-md p-4 sm:p-8">
            ...
            >
              <div className="px-10 py-6 border-b border-slate-100 flex items-center justify-between bg-white relative z-10 no-print">
                <div>
                  <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight flex items-center gap-2">
                    <Eye className="w-5 h-5 text-indigo-600" /> Prévia da
                    Listagem
                  </h3>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">
                    Visualize como ficará a folha de chamada
                  </p>
                </div>
                <div className="flex items-center gap-3">
                  <button
                  ...
                  <button
                    onClick={() => {
                      const element =
                        document.querySelector(".print-area-class");
                      if (!element || !(element instanceof HTMLElement)) return;
              ...
              <div className="flex-1 overflow-y-auto p-12 bg-slate-200/50 flex justify-center custom-scrollbar">
                <div className="print-container print-area-class bg-white w-full max-w-[297mm] h-[209mm] shadow-2xl p-[1.5cm] relative text-slate-900 overflow-hidden">
                  ...
                  <table className="w-full text-[10px] border-collapse">
                    <thead>
                      <tr>
                        <th className="border border-slate-300 p-2 text-left uppercase font-black w-8">
                          #
                        </th>
                        <th className="border border-slate-300 p-2 text-left uppercase font-black">
                          Aluno
                        </th>
                        <th className="border border-slate-300 p-2 text-left uppercase font-black w-24">
                          CPF
                        </th>
                        <th className="border border-slate-300 p-2 text-center uppercase font-black w-8">
                          P
                        </th>
                        <th className="border border-slate-300 p-2 text-center uppercase font-black w-8">
                          F
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/40 backdrop-blur-md p-4 sm:p-8">
            ...
            >
              <div className="px-10 py-6 border-b border-slate-100 flex items-center justify-between bg-white relative z-10 no-print">
                <div>
                  <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight flex items-center gap-2">
                    <FileText className="w-5 h-5 text-indigo-600" /> Relatório
                    Detalhado da Turma
                  </h3>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">
                    Dados cadastrais completos de todos os alunos aprovados
                  </p>
                </div>
                <div className="flex items-center gap-3">
                  <button
              ...
              <div className="flex-1 overflow-y-auto p-12 bg-slate-200/50 flex justify-center custom-scrollbar">
                <div className="print-container print-area-full-class bg-white w-[297mm] h-[209mm] shadow-2xl p-[1cm] relative text-slate-900 border border-slate-100 overflow-hidden">
                  <div className="flex items-center justify-between border-b-2 border-slate-900 pb-4 mb-6">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 bg-slate-900 rounded flex items-center justify-center text-white font-black text-lg">
                        CB
                      </div>
                      <div>
                        <h1 className="text-xl font-black uppercase tracking-tighter">
                          Relatório de Matrículas Ativas
                        </h1>
                        <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">
                          Turma: {viewingClass.title} | Polo:{" "}
                          {viewingClass.polo}
                        </p>
                      </div>
                    </div>
                  ...
                  <table className="w-full text-[8.5px] border-collapse">
                    <thead className="bg-slate-50">
                      <tr>
                        <th className="border border-slate-300 p-2 text-left uppercase font-black w-6">
                          #
                        </th>
                        <th className="border border-slate-300 p-2 text-left uppercase font-black">
                          Aluno / Filiação
                        </th>
                        <th className="border border-slate-300 p-2 text-left uppercase font-black w-20">
                          Documentos
                        </th>
                        <th className="border border-slate-300 p-2 text-left uppercase font-black w-18">
                          Contato
                        </th>
                        <th className="border border-slate-300 p-2 text-left uppercase font-black">
                          Escola Atual
                          ...
                          <tr key={enrollment.id} className="align-top">
                            ...
                            <td className="border border-slate-300 p-2">
                              ...
                              <div className="text-[7.5px] text-slate-500 space-y-0.5">
                                <p>
                                  ...
                                  {enrollment.studentData.motherName}
                                </p>
                                <p>
                                  <span className="font-bold">Pai:</span>{" "}
                                  {enrollment.studentData.fatherName}
                                </p>
                                <p>
                                  <span className="font-bold">Resp.:</span>{" "}
                                  {enrollment.studentData.guardianName || "-"}
                                </p>
                              </div>
                            </td>
                            <td className="border border-slate-300 p-2 space-y-1">
                              ...
                              <p>
                                ...
                                {enrollment.studentData.rg}
                              </p>
                              <p>
                                <span className="font-bold">Nasc:</span>{" "}
                                {enrollment.studentData.birthDate}
                              </p>
                            </td>
                            <td className="border border-slate-300 p-2">
                            ...
                            <td className="border border-slate-300 p-2 space-y-1">
                              <p className="font-black uppercase">
                                {enrollment.studentData.schoolName}
                              </p>
                              <p>
                                {enrollment.studentData.schoolGrade} •{" "}
                                {enrollment.studentData.shift}
                              </p>
                            </td>
                            <td className="border border-slate-300 p-2 space-y-1 text-[7.5px]">
                              <p className="font-bold uppercase">
                                {enrollment.studentData.address},{" "}
                                {enrollment.studentData.houseNumber}
                              </p>
                              <p className="uppercase text-slate-500">
                                {enrollment.studentData.neighborhood}
                              </p>
                              <div className="flex gap-2 mt-1 pt-1 border-t border-slate-100">
                                <p>
                                  ...
                                  <span
                                    className={
                                      isIrregular(
                                        enrollment.studentData.paysElectricity,
                                      )
                                        ? "text-rose-600"
                                        : ""
                                    }
                                ...
                                <p>
                                  ...
                                  <span
                                    className={
                                      isIrregular(
                                        enrollment.studentData.paysWater,
                                      )
                                        ? "text-rose-600"
                                        : ""
                                    }
                              ...
                              {enrollment.studentData.landmark && (
                                <p className="italic text-[7px] text-slate-400 mt-0.5">
                                  Ref: {enrollment.studentData.landmark}
                                </p>
                              )}
                            </td>
                          </tr>
                  ...
                  <div className="mt-8 pt-6 border-t border-slate-100 flex justify-between">
                    <p className="text-[8px] font-bold text-slate-300">
                      SISTEMA DE GESTÃO BANTU - ACESSO ADMINISTRATIVO
                    </p>
                    <p className="text-[8px] font-bold text-slate-300">
                      Página 1 de 1
                    </p>
      ...
      <AnimatePresence>
        {viewingEnrollment &&
          !isPreviewingPrint &&
          !isPreviewingFullEnrollmentPrint && (
            <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
              ...
              >
                <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50 no-print">
                  <div>
                    ...
                    <p className="text-slate-500 font-bold text-[10px] uppercase tracking-widest">
                      Inscrição ID: {viewingEnrollment.id.slice(0, 8)}
                    </p>
                  </div>
                  <button
                    onClick={() => setViewingEnrollment(null)}
                ...
                <div className="flex flex-wrap justify-between px-8 pt-6 no-print gap-4">
                  <div className="flex gap-2 flex-1">
                    ...
                    <button
                      onClick={() => setIsPreviewingFullEnrollmentPrint(true)}
                ...
                <div className="p-8 overflow-y-auto space-y-10 custom-scrollbar">
                  ...
                  <section>
                    <h4 className="label-caps mb-6 text-indigo-600 flex items-center gap-2">
                      <GraduationCap className="w-4 h-4" /> Cursos Matriculados
                      (
                      {groupedEnrollments[
                        viewingEnrollment.studentData.cpf ||
                          viewingEnrollment.studentData.fullName
                    ...
                    <div className="space-y-4">
                      {(
                        ...
                        const c = courses.find(
                          (course) => course.id === e.courseId,
                        );
                        return (
                          <div
                            key={e.id}
                            className="flex items-center justify-between p-4 bg-slate-50 border-2 border-slate-100 rounded-2xl group hover:border-indigo-100 transition-all"
                          >
                            <div>
                              ...
                              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-0.5">
                                Polo: {e.polo} • Matrícula: {e.id.slice(0, 8)}
                              </p>
                            </div>
                            <div className="flex items-center gap-4">
                                ...
                                {e.status === "pending" && (
                                  <>
                                    <button
                                      ...
                                      className="p-2 text-emerald-600 hover:bg-white rounded-lg transition-all"
                                      title="Aprovar"
                                    >
                  ...
                  <section>
                    ...
                    {(() => {
                      const studentEnrollments = groupedEnrollments[
                        viewingEnrollment.studentData.cpf ||
                          viewingEnrollment.studentData.fullName
                      ] || [viewingEnrollment];
                      const studentKey =
                        viewingEnrollment.studentId &&
                        viewingEnrollment.studentId !== "presential_referral"
                          ? viewingEnrollment.studentId
                          : viewingEnrollment.studentData.cpf
                            ? viewingEnrollment.studentData.cpf.replace(
                                /\D/g,
                                "",
                              )
                            : viewingEnrollment.studentData.fullName
                      ...
                      const scholarship = scholarships.find(
                        (s) =>
                          s.studentId === studentKey ||
                          s.studentId === viewingEnrollment.studentId,
                      );
                      ...
                      return (
                        <div
                          className={`mb-8 p-8 rounded-3xl border-2 transition-all overflow-hidden relative ${
                            status === "active"
                              ? "bg-emerald-50/50 border-emerald-100 shadow-xl shadow-emerald-50/50"
                              : status === "eligible"
                                ? "bg-indigo-50/50 border-indigo-100 shadow-xl shadow-indigo-50/50"
                                : status === "suspended"
                                  ? "bg-rose-50/50 border-rose-100 shadow-xl shadow-rose-50/50"
                        ...
                        >
                          ...
                          <div className="relative z-10">
                            <div className="flex items-start justify-between mb-6">
                              <div className="flex items-center gap-4">
                                <div
                                  className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-lg transition-transform hover:scale-110 ${
                                    status === "active"
                              ...
                              <div className="flex flex-wrap gap-2">
                                ...
                                scholarshipConfig.benefits.length > 0 ? (
                                  scholarshipConfig.benefits.map((b, i) => (
                                    <span
                                      key={i}
                                      className={`px-3 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest border transition-all hover:scale-105 ${
                                        status === "active"
                            ...
                            {status === "eligible" && (
                              <div className="mt-4 flex items-center gap-3 bg-indigo-600/5 p-3 rounded-xl border border-indigo-100/50">
                                ...
                                <p className="text-[9px] font-bold text-indigo-700 uppercase tracking-tight">
                                  Aguardando ativação administrativa
                                </p>
                              </div>
                            )}

                            {status === "suspended" && (
                              <div className="mt-4 flex items-center gap-3 bg-rose-600/5 p-3 rounded-xl border border-rose-100/50">
                                <AlertCircle className="w-4 h-4 text-rose-600 animate-pulse" />
                                <p className="text-[9px] font-bold text-rose-700 uppercase tracking-tight">
                                  Atenção: Esta bolsa foi suspensa pela
                                  administração e não está ativa.
                                </p>
                              </div>
                            )}
                          </div>
                    ...
                    <div className="grid grid-cols-2 gap-x-12 gap-y-6">
                      ...
                      <div>
                        ...
                        <p className="text-sm font-bold text-slate-900">
                          CPF: {viewingEnrollment.studentData.cpf} / RG:{" "}
                          {viewingEnrollment.studentData.rg}
                        </p>
                      </div>
                    </div>
                  ...
                  {parseInt(viewingEnrollment.studentData.age) < 18 && (
                    <section className="bg-amber-50/30 p-6 rounded-2xl border-2 border-amber-100/50">
                      ...
                      <div className="space-y-6">
                        <div>
                          <p className="text-[10px] text-amber-600/70 font-black uppercase tracking-widest mb-1">
                            Nome do Responsável
                          </p>
                          <p className="text-sm font-bold text-slate-900">
                            {viewingEnrollment.studentData.guardianName}
                        ...
                        <div className="grid grid-cols-2 gap-12">
                          <div>
                            ...
                            <p className="text-sm font-bold text-slate-900">
                              {viewingEnrollment.studentData.guardianCpf}
                            </p>
                          </div>
                          <div>
                            <p className="text-[10px] text-amber-600/70 font-black uppercase tracking-widest mb-1">
                            ...
                            <p className="text-sm font-bold text-slate-900">
                              {viewingEnrollment.studentData.guardianRg}
                            </p>
                          </div>
                        </div>
                      </div>
                  ...
                  <section>
                    ...
                    <div className="grid grid-cols-2 gap-x-12 gap-y-6">
                      ...
                      <div>
                        ...
                        <p className="text-sm font-bold text-slate-900">
                          {viewingEnrollment.studentData.shift}
                          {viewingEnrollment.studentData.shift === "Integral" &&
                            viewingEnrollment.studentData.integralDays &&
                            ` (${viewingEnrollment.studentData.integralDays})`}
                        </p>
                      </div>
                  ...
                  <section>
                    ...
                    <div className="grid grid-cols-2 gap-x-12 gap-y-6">
                      ...
                      <div>
                        ...
                        <p className="text-sm font-bold text-slate-900">
                          {viewingEnrollment.studentData.householdCount} pessoas
                          / {viewingEnrollment.studentData.incomePerCapita}
                        </p>
                      </div>
                      <div className="col-span-2">
                        ...
                        <p className="text-sm font-bold text-slate-900">
                          {viewingEnrollment.studentData.receivesBenefit ===
                          "SIM"
                            ? `Sim: ${viewingEnrollment.studentData.benefitDetail}`
                      ...
                      <div className="col-span-2 grid grid-cols-2 gap-12 pt-4 border-t border-slate-100">
                        <div>
                          ...
                          <p
                            className={`text-sm font-bold ${isIrregular(viewingEnrollment.studentData.paysElectricity) ? "text-rose-600" : "text-slate-900"}`}
                          >
                            {formatSaneamento(
                              viewingEnrollment.studentData.paysElectricity,
                            )}
                        ...
                        <div>
                          ...
                          <p
                            className={`text-sm font-bold ${isIrregular(viewingEnrollment.studentData.paysWater) ? "text-rose-600" : "text-slate-900"}`}
                          >
                            {formatSaneamento(
                              viewingEnrollment.studentData.paysWater,
                            )}
                ...
                <div className="p-8 bg-slate-50 border-t border-slate-100 flex gap-4 no-print">
                  {viewingEnrollment.status === "pending" && (
                    <>
                      <button
                        onClick={() =>
                          handleStatusUpdate(viewingEnrollment.id, "approved")
                        }
                      ...
                      <button
                        onClick={() =>
                          handleStatusUpdate(viewingEnrollment.id, "rejected")
                        }
          ...
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            >
              <div className="p-10 border-b border-slate-100 flex justify-between items-center bg-slate-50/50 no-print">
                ...
                <button
                  onClick={() => setViewingClass(null)}
                  className="p-3 hover:bg-white hover:shadow-lg rounded-2xl transition-all text-slate-400 border border-transparent hover:border-slate-100"
                >
                  <X className="w-6 h-6" />
                </button>
              </div>
                ...
                <table className="w-full text-left font-sans text-sm">
                  <thead>
                    <tr className="border-b-2 border-slate-100">
                      <th className="pb-4 text-[11px] font-black uppercase tracking-[0.2em] text-slate-400">
                        Aluno
                      </th>
                      <th className="pb-4 text-[11px] font-black uppercase tracking-[0.2em] text-slate-400">
                        CPF / RG
                      ...
                      <th className="pb-4 text-[11px] font-black uppercase tracking-[0.2em] text-slate-400 text-right">
                        Status
                      </th>
                    </tr>
                  </thead>
                      ...
                      .map((enrollment) => (
                        ...
                        >
                          <td className="py-6 pr-4">
                            <p className="font-black text-slate-950 text-base leading-tight">
                              {enrollment.studentData.fullName}
                            </p>
                            <p className="text-[10px] text-slate-400 font-bold uppercase mt-1">
                              Data: {enrollment.studentData.enrollmentDate}
                            </p>
                          </td>
                          <td className="py-6 pr-4">
                            <p className="text-slate-600 font-medium">
                              CPF: {enrollment.studentData.cpf}
                            </p>
                            <p className="text-[10px] text-slate-400">
                              RG: {enrollment.studentData.rg}
                            </p>
                          </td>
                          <td className="py-6 pr-4">
                          ...
                          <td className="py-6 pr-4">
                            <p className="text-slate-600 font-medium max-w-[150px] truncate">
                              {enrollment.studentData.schoolName}
                            </p>
                            <p className="text-[10px] text-slate-400">
                              {enrollment.studentData.schoolGrade}
          ...
          >
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-400 font-medium">
                Sua matrícula está em análise técnica.
              </p>
            </div>
          </motion.div>
        )}
      ...
      <AnimatePresence>
        {showPrivacyNotice && <PrivacyModal onAccept={handleAcceptTerms} />}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {showTeacherPanel && (
          <TeacherPanelModal onClose={() => setShowTeacherPanel(false)} />
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {confirmDialog && confirmDialog.isOpen && (
          <div className="fixed inset-0 z-[999] flex items-center justify-center p-4">
            ...
            >
              <div
                className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-5 ${confirmDialog.isDangerous ? "bg-rose-50 text-rose-600 border border-rose-100" : "bg-indigo-50 text-indigo-600 border border-indigo-100"}`}
              >
