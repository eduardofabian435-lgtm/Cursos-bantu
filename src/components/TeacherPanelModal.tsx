...
export const TeacherPanelModal: React.FC<TeacherPanelModalProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<
    "classes" | "attendance" | "history" | "grades"
  >("classes");
  ...
  const [editingGrades, setEditingGrades] = useState<{
    [studentId: string]: { a1: string; a2: string };
  }>({});
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [allCourses, allEnrollments, allScholarships, config] =
        await Promise.all([
          getCourses(),
          getAllEnrollments(),
          getStudentScholarships(),
  ...
  const handleSaveAttendance = async () => {
    ...
    try {
      await saveAttendance({
        ...
        date: attendanceDate,
        teacherId: user.uid,
        attendees: filteredAttendance.map(
              ...
              <div className="max-w-2xl mx-auto">
                <div className="flex items-center justify-between mb-8">
                  ...
                  <div className="bg-emerald-50 text-emerald-600 px-4 py-2 rounded-2xl border border-emerald-100 flex items-center gap-2 text-xs font-black uppercase tracking-widest shrink-0">
                    ...
                    {
                      attendanceList.filter((a) => a.status === "present")
                        .length
                    }{" "}
                    Presentes
                  </div>
                ...
                <div className="space-y-3 mb-10 max-h-[400px] overflow-y-auto px-1 -mx-1 custom-scrollbar">
                  {filteredAttendance.map((item) => (
                    ...
                    >
                      <div className="flex items-center gap-4">
                        <div
                          className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black transition-all ${item.status === "present" ? "bg-emerald-600 text-white shadow-lg shadow-emerald-200" : "bg-slate-200 text-slate-400"}`}
                        >
                        ...
                        <div className="text-left">
                          <div className="flex items-center gap-2">
                            ...
                            {(() => {
                              const studentKey =
                                item.studentId &&
                                item.studentId !== "presential_referral"
                                  ? item.studentId
                                  : item.studentCpf
                                    ? item.studentCpf.replace(/\D/g, "")
                                    : item.studentName
                                        .toLowerCase()
                                        .trim()
                                        .replace(/[^a-z0-9]/g, "");
                              ...
                              const studentEnrs = enrollments.filter((e) => {
                                const eKey =
                                  e.studentId &&
                                  e.studentId !== "presential_referral"
                                    ? e.studentId
                                    : e.studentData?.cpf
                                      ...
                                      : e.studentData?.fullName
                                          ...
                                          .replace(/[^a-z0-9]/g, "");
                                return (
                                  eKey === studentKey && e.status === "approved"
                                );
                              });
                              if (scholarship?.status === "active") {
                                return (
                                  <div
                                    className="flex items-center gap-1 bg-emerald-50 text-emerald-600 px-1.5 py-0.5 rounded border border-emerald-100"
                              ...
                              } else if (scholarship?.status === "suspended") {
                                return (
                                  <div
                                    className="flex items-center gap-1 bg-rose-50 text-rose-600 px-1.5 py-0.5 rounded border border-rose-100"
                          ...
                          <p
                            className={`text-[10px] font-black uppercase tracking-widest ${item.status === "present" ? "text-emerald-500" : "text-slate-400"}`}
                          >
                      ...
                      <div
                        className={`w-8 h-8 rounded-full flex items-center justify-center transition-all ${item.status === "present" ? "bg-emerald-100 text-emerald-600" : "bg-slate-200 text-slate-400"}`}
                      >
                        {item.status === "present" ? (
                ...
                <div className="bg-white rounded-[2rem] border border-slate-100 shadow-xl shadow-slate-200/50 overflow-hidden">
                  <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-slate-50">
                      {selectedCourse &&
                        getStudentsForCourse(selectedCourse.id).map(
                          (student) => {
                              ...
                              >
                                    ...
                                    {(() => {
                                      const studentKey =
                                        ...
                                        student.studentId !==
                                          ...
                                          ? student.studentId
                                          : student.studentData?.cpf
                                            ? student.studentData.cpf.replace(
                                                /\D/g,
                                                "",
                                              )
                                            : student.studentData?.fullName
                                      ...
                                      const studentEnrs = enrollments.filter(
                                        (e) => {
                                          const eKey =
                                            e.studentId &&
                                            e.studentId !==
                                      ...
                                      if (scholarship?.status === "active") {
                                        return (
                                          <div
                                            className="bg-emerald-50 text-emerald-600 p-1.5 rounded-lg border border-emerald-100"
                                            title="Bolsista Ativo"
                                          >
                                        ...
                                        return (
                                          <div
                                            className="bg-rose-50 text-rose-600 p-1.5 rounded-lg border border-rose-100"
                                            title="Bolsa Suspensa"
                                          >
                                ...
                                <td className="px-6 py-4 text-center">
                                  <div className="inline-flex flex-col items-center">
                                    <span
                                      className={`text-xs font-black ${freq < 75 ? "text-rose-600" : "text-emerald-600"}`}
                                    >
                                      {freq}%
                                    </span>
                                    <div className="w-16 h-1 bg-slate-100 rounded-full mt-1 overflow-hidden">
                                      <div
                                        className={`h-full ${freq < 75 ? "bg-rose-500" : "bg-emerald-500"}`}
                                        style={{ width: `${freq}%` }}
                                      />
                                    </div>
                                  </div>
                                ...
                                <td className="px-6 py-4 text-center">
                                  <span
                                    className={`text-sm font-black ${avg < 6 ? "text-rose-600" : "text-indigo-600"}`}
                                  >
                                    {avg.toFixed(1)}
                                  </span>
                                </td>
                        ...
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-[9px] text-slate-400 font-bold uppercase mt-1">
                              De {record.attendees.length} Alunos
                            </p>
                          </div>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start justify-between mb-4">
                        <div
                          className={`p-3 rounded-2xl transition-colors ${selectedCourse?.id === course.id ? "bg-indigo-600 text-white" : "bg-slate-100 text-slate-400 group-hover:bg-indigo-50 group-hover:text-indigo-600"}`}
                        >
                      ...
                      <h4 className="text-lg font-black text-slate-900 uppercase tracking-tighter mb-1 leading-tight">
                        {course.title}
                      </h4>
              ...
              {selectedCourse ? (
                <div className="flex flex-col h-full">
                  <div className="p-8 border-b border-slate-50">
                    ...
                    <h3 className="text-2xl font-black text-slate-900 uppercase tracking-tighter leading-[0.9]">
                      {selectedCourse.title}
                    </h3>
                    <div className="grid grid-cols-2 gap-4 mt-8">
                      <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 text-center">
                        ...
                        <p className="text-2xl font-black text-slate-900 leading-none">
                          {getStudentsForCourse(selectedCourse.id).length}
                        </p>
                      </div>
                      <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 text-center">
                        <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">
                    ...
                    <div className="space-y-3">
                      {getStudentsForCourse(selectedCourse.id).map(
                        (student) => (
                          <div
                            key={student.id}
                            className="flex items-center gap-4 p-4 rounded-2xl bg-white border border-slate-100 hover:border-slate-200 transition-all group"
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2">
                                ...
                                {(() => {
                                  const studentKey =
                                    student.studentId &&
                                    student.studentId !== "presential_referral"
                                      ? student.studentId
                                      : student.studentData?.cpf
                                        ? student.studentData.cpf.replace(
                                            /\D/g,
                ...
                <div className="h-full flex flex-col items-center justify-center text-center p-12 text-slate-400">
                  ...
                  <p className="text-[10px] font-bold uppercase tracking-widest leading-relaxed mt-2 max-w-[200px]">
                    Escolha uma turma ao lado para gerenciar alunos e diário de
                    classe.
                  </p>
                </div>
              )}
